...
class InvokerReactive(
  ...
  // probing agent. configure addr to connect to
  private val probingAgentConfig = ProbingAgentConfig(
    addr = "172.17.0.1", 
    httpPort = 8080, 
    listeningHttpPort = 50051
  )

  private val probingAgentBridge: ActorRef = 
    actorSystem.actorOf(
      ProbingAgentBridge.props(
        probingAgentConfig.addr,
        probingAgentConfig.httpPort,
        probingAgentConfig.listeningHttpPort
      ),
      name = "probing-agent-bridge"
    ) 
  ...
  private val childFactory = (f: ActorRefFactory) =>
    f.actorOf(
      ContainerProxy
        .props(containerFactory.createContainer, 
          ack, 
          store, 
          collectLogs, 
          instance, 
          poolConfig, 
          probingAgentBridge = Some(probingAgentBridge)))
