...
// Probing state for container resize
sealed trait ProbePhase
case object ProbeIdle extends ProbePhase // not probing, waiting for next run
case object ProbeRunning extends ProbePhase // probing 
case object ProbeCooldown extends ProbePhase // cooldown after failed probing
case object ProbeDisabled extends ProbePhase // permanently disabled
...
case object StartProbe
case object StopProbe
...
class ContainerProxy(factory: (TransactionId,
                     ...
                     probingAgentBridge: Option[ActorRef])
  ...
  when(Ready, stateTimeout = pauseGrace) {
    ...
    case Event(UpdateProbe, data: WarmedData) =>
      val id = data.container.containerId.asString
      probingAgentBridge.foreach(_ ! UpdateProbing(id))
      logging.debug(this, s"Touched container $id")
      stay using data
  ...
  when(Pausing) {
    ...
    case Event(UpdateMemoryLimit(newLimit), data: WarmedData) =>
      updateTimeout(newLimit)
      logging.debug(this, s"[PAUSING] downsizing container ${data.container.containerId.asString} to ${newLimit.toMB} MB")

      // update container data
      val newData = data.withMemoryLimit(newLimit)

      // paused container also lives in the free pool.
      logging.debug(this, s"[PAUSING](updateMemory) activation count: ${activeCount}")
      //context.parent ! NeedWork(newData)
      context.parent ! ContainerUpdated(newData)
      stay using newData
  ...
  when(Paused, stateTimeout = unusedTimeout) {
    ...
    case Event(UpdateMemoryLimit(newLimit), data: WarmedData) =>
      updateTimeout(newLimit)
      logging.debug(this, s"[PAUSED] downsizing container ${data.container.containerId.asString} to ${newLimit.toMB} MB")

      // update container data
      val newData = data.withMemoryLimit(newLimit)

      // paused container also lives in the free pool.
      logging.debug(this, s"[PAUSED](updateMemory) activation count: ${activeCount}")
      //context.parent ! NeedWork(newData)
      context.parent ! ContainerUpdated(newData)
    ...
    case Event(StateTimeout | Remove, data: WarmedData) =>
      logging.debug(this, s"[PAUSED] container ${data.container.containerId.asString} timed out after ${myUnusedTimeout.toMillis}ms")
  ...
  def requestWork(newData: WarmedData): Boolean = {
    logging.debug(this, s"[REQUESTWORK] activation count: ${activeCount}")
  ...
  def destroyContainer(newData: ContainerStarted,
    ...
    // stop probing
    val id = newData.container.containerId.asString
    probingAgentBridge.foreach(_ ! StopProbing(id))
