 ...
 * @param probingAgentConfig config for the ProbingAgentBridge
 * @param probingAgentBridge actor reference to the ProbingAgentBridge
 */
class ContainerPool(childFactory: ActorRefFactory => ActorRef,
                    feed: ActorRef,
                    prewarmConfig: List[PrewarmingConfig] = List.empty,
                    poolConfig: ContainerPoolConfig,
                    probingAgentConfig: ProbingAgentConfig,
                    probingAgentBridge: ActorRef)(implicit val logging: Logging)
    extends Actor {
  ...
  private var downsizedLimits = immutable.Map.empty[String, ByteSize]
  ...
  def receive: Receive = {
    ...
    case ContainerRemoved(replacePrewarm) =>
      ...
      freePool.get(sender()).foreach {
        case w: WarmedData =>
          freePool = freePool - sender()
          downsizedLimits = downsizedLimits - (w.container.containerId.asString) // remove the downsized limit
        case _ =>
          freePool = freePool - sender()
      }
...
object ContainerPool {
  case class ContainerMemoryDownsized(containerId: String, newLimitBytes: ByteSize)
  case class CommitsUpdate(commits: List[ContainerMemoryDownsized])
  ...
  def props(factory: ActorRefFactory => ActorRef,
            ...
            prewarmConfig: List[PrewarmingConfig] = List.empty,
            probingAgentConfig: ProbingAgentConfig,
            probingAgentBridge: ActorRef)(implicit logging: Logging) =
    Props(new ContainerPool(factory, feed, prewarmConfig, poolConfig, probingAgentConfig, probingAgentBridge ))
}
...
/** Contains settings for ProbingAgentBridge */
case class ProbingAgentConfig(addr: String,
                               httpPort: Int,
